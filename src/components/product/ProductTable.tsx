...
type TProductTable = { title: string };

const ProductTable: React.FC<TProductTable> = ({ title }) => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const page = Number(searchParams.get("page")) || config.pagination.PAGE;
  const limit = Number(searchParams.get("limit")) || config.pagination.LIMIT;
  const { data, isLoading, error } = useQuery({
    queryKey: ["products", { page, limit }],
    queryFn: () => getProducts(page, config.pagination.LIMIT),
  });
    ...
    <>
      ...
      <div className="flex flex-col">
        <div className="grid grid-cols-7 border-t border-stroke p-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5">
          ...
          <div className="col-span-2 sm:col-span-1 flex items-center justify-end text-end">
